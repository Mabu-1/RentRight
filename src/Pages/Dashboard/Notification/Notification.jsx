        ...
        <div className="max-w-3xl mx-auto p-4">
            ...
            <div className="flex justify-center items-center mb-6">
                <div className="w-[120px] sm:w-[150px] border border-gray-300 rounded-lg px-3 py-2 font-bold">
                    <select
                        className=" mabu-text2 focus:outline-none bg-transparent w-full"
                        value={read}
                        onChange={(e) => setRead(e.target.value)}
                    >
                        <option className=" mabu-text3" value="All">All</option>
                        <option className=" mabu-text3"value="read">Read</option>
                        <option className=" mabu-text3" value="unread">Unread</option>
                    </select>
                </div>
            </div>
            <div className="space-y-4  sm:p-4">
                {filteredNotifications.length > 0 ? (
                    filteredNotifications.map((notification, index) => (
                        ...
                        >
                            <h2 className="mabu-text text-lg sm:text-xl font-bold">{notification.title}</h2>
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-2 sm:mt-0">
                                <p className="mabu-text3 text-sm sm:text-base">{notification.message}</p>
                                <button
